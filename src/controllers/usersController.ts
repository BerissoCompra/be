...
class UsersController {
  public async iniciarSesion(req: Request, res: Response) {
    try {
      ...
      if (!usuarioExiste)
        return await res.status(404).json({ msg: 'El usuario no existe' });
  ...
  public async iniciarSesionCliente(req: Request, res: Response) {
    try {
      ...
      if (!usuarioExiste)
        return res
          .status(404)
          .json({ msg: 'El usuario y/o contraseña son incorrectos' });
      ...
      return res
        .status(200)
        .json({ token, finalizoTutorial: usuarioExiste.finalizoTutorial });
    } catch (error) {
      return await res.status(500).json({ msg: 'Error al autenticar' });
    }
  }
  ...
  public async crearUsuario(req: Request, res: Response) {
    try {
      ...
      if (usuarioExiste.length > 0) {
        return res
          .status(200)
          .json({ msg: 'El email ingresado ya se encuentra registrado.' });
      } else {
        ...
        if (usuarioRegistrado) {
          ...
          await sendEmail(
            'Activar Cuenta | Responsable de Comercio',
            email,
  ...
  public async crearUsuarioCliente(req: Request, res: Response) {
    try {
      ...
      const usuarioExiste = await Cliente.findOne({
      ...
      if (!terminos)
        return res
          .status(404)
          .json({ msg: 'Debe aceptar los Términos y Condiciones.' });

      const nuevoUsuario = new Cliente(req.body);
      console.log(req.body);
      nuevoUsuario.password = await Cliente.encryptPassword(
        nuevoUsuario.password,
      );
      const usuarioRegistrado = await nuevoUsuario.save();
      if (usuarioRegistrado) {
  ...
  public async VerificarCuenta(req: Request, res: Response) {
    ...
    if (usuarioExiste) {
      if (usuarioExiste.emailActivado === true) {
        return res
          .status(404)
          .json({ msg: 'Este usuario ya se encuentra activado.' });
      } else {
  ...
  public async verificarUsuario(req: any, res: Response) {
    ...
    if (cliente) {
      return res
        .status(200)
        .json({ valido: true, tutorial: cliente.finalizoTutorial });
    } else {
      return res
        .status(200)
        .json({ valido: false, tutorial: cliente.finalizoTutorial });
    }
  }
  ...
  public async finalizarTutorial(req: any, res: Response) {
    try {
      const { uid } = req.data;
      const actualizarCliente = await Cliente.findByIdAndUpdate(uid, {
        finalizoTutorial: true,
      });
  ...
  public async addFavorito(req: Request, res: Response) {
    try {
      ...
      const existe = cliente.favoritos.filter(
        (fav: string) => fav === comercioId,
      );

      if (existe.length > 0) {
        nuevosFavoritos = cliente.favoritos.filter(
          (fav: string) => fav != comercioId,
        );
        respuesta = false;
      } else {
        ...
        cliente.favoritos.map(async (comercioId: string) => {
          await Comercio.findById(comercioId)
            ...
            .catch((err: any) => {
              ...
              return res
                .status(404)
                .json({ msg: 'Error al obtener comercio fav.' });
            });
        }),
      );
  ...
  public async verificarCodigo(req: Request, res: Response) {
    ...
    const buscarCodigo = await CodigosRecuperacion.findOne({
      codigo,
      email,
    ...
    if (codigoRecuperacion) {
      if (rol === 'comercio') {
        const usuario = await Usuario.findOne({
          email: codigoRecuperacion.email,
        });
        ...
        if (usuario) {
          ...
          await CodigosRecuperacion.deleteOne({
            codigo,
            email: codigoRecuperacion.email,
          });
